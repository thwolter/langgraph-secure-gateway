...
def build(
    ...
    base_image: str = typer.Option("langchain/langgraph-api:3.11-wolfi", "--base-image"),
    api_version: str | None = typer.Option(None, "--api-version"),
    engine_runtime_mode: str = typer.Option("combined_queue_worker", "--engine-runtime-mode"),
...
def create_admin_user(
    ...
    action = create_or_update_admin_user(username=username, password=password, inactive=inactive)
...
def init_deploy(
    ...
    if langgraph_config_path.exists():
        ...
        if security_dependency not in dependencies:
            ...
            langgraph_config_path.write_text(json.dumps(data, indent=2) + "\n", encoding="utf-8")
